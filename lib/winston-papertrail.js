...
Papertrail.prototype.log = function (info, callback) {
    ...
    this.sendMessage(this.hostname, this.program, level, output, config, callback);
 ...
 */
Papertrail.prototype.sendMessage = function (hostname, program, level, message, config, callback) {
    var self = this,
        lines = [],
        msg = '',
        gap = '';
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        let formatted = self.logFormat(self.colorize ? winston.config.colorize(level) : level, gap + lines[i]);
        let prepend = '';

        if (config.tags) {
            Object.keys(config.tags).forEach(key => prepend += ` ${key}=${config.tags[key]}`);
        }
        ...
        msg += self.producer.produce({
            ...
            host: hostname,
            appName: program,
            date: config.timestamp || new Date(),
            message: `${prepend} ${formatted}`.trim(),
